                    ...
                    <div class="nk-block nk-block-lg">
                        <div class="dt-export-buttons d-flex align-center mb-3 d-flex justify-content-end">
                            ...
                            <div class="dt-buttons btn-group flex-wrap">
                                <button id="download-pdf-onem" class="btn btn-secondary buttons-pdf buttons-html5 onem_info" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Pdf" data-bs-original-title="Pdf" tabindex="0" aria-controls="DataTables_Table_2" type="button">
                                    <span>PDF</span>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="row gy-4 mb-4 d-flex align-conten">
                            ...
                            <div class="col-lg-2 col-sm-4">
                                <button href="#" id="onemData" class="btn-lg btn btn-primary" onclick="filterData()">
                                    <span>Filtrer</span>
...
<script>
    ...
    document.getElementById('download-pdf-onem').addEventListener('click', async function(event) {
        ...
        try {
            let response = await fetch('<?= URL; ?>company/generateonemPdfAjax', {
                method: 'POST',
                headers: {
...
<script>
    ...
    document.getElementById('onemData').addEventListener('click', function() {
        var year = document.getElementById('year-select').value;
        var month = document.getElementById('month-select').value;

        var xhr = new XMLHttpRequest();
        xhr.open('POST', '<?= URL; ?>company/onem_searchData', true);
...
<script>
    function filterData() {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', '<?= URL; ?>company/onem_search', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onload = function() {
            if (this.status == 200) {
